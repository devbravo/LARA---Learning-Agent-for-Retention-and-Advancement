...
from src.core.sm2 import get_due_topics
...
def _make_db(topics: list[dict]) -> str:
...
def test_get_due_topics_returns_topics_due_today():
    ...
    result = get_due_topics(db_path=path)
    assert len(result) == 1
...
def test_get_due_topics_target_date_tomorrow():
    ...
    result = get_due_topics(db_path=path, target_date=tomorrow)
    assert len(result) == 1
